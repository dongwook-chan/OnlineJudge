...
bool flower[MAX_GARDEN_SIZE][MAX_GARDEN_SIZE];
int flower_ctr;
int max_flower_ctr;
void propagate() {
    ...
    for(int t = 1; colored_tile[GREEN].size() && colored_tile[RED].size(); ++t) {  // 매 초 NOTE: 큐가 비었으면 탐색 종료해야...
        for(int c = 0; c < 2; ++c) {    // 색깔 (green, red)
            ...
            while(tile_ctr--) {
                ...
                if(flower[y][x]) continue; // // NOTE: 꽃이 핀 RED 좌표는 push가 안 되지만 GREEN좌표는 여전히 들어가버림... pop할 때 또 한 번 검사!  
                ...
                for(int d = 0; d < 4; ++d) {
                    ...
                    if(garden[ny][nx] == LAKE) continue; // NOTE: 땅 아닌 경우 제외 안 함 ㅜㅜ..
                    // NOTE: t를 단 한 번 허용해줘야... 남이 도달했던 첫번째 t를 도달할 경우 꽃이 핌...
                    if(visited_timestamp[!c][ny][nx] > -1 && visited_timestamp[!c][ny][nx] < t) continue;  // 배양액은 매 초마다 이전에 배양액이 도달한 적이 없는 인접한 땅으로 퍼져간다.
                    if(visited_timestamp[c][ny][nx] > -1 && visited_timestamp[c][ny][nx] <= t) continue;  // 배양액은 매 초마다 이전에 배양액이 도달한 적이 없는 인접한 땅으로 퍼져간다.
                    ...
                    else{   // NOTE: 꽃이 핀 RED 좌표는 push가 안 되지만 GREEN좌표는 여전히 들어가버림... pop할 때 또 한 번 검사!  
