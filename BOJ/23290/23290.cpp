...
void print_fish_smell(int s) {
#ifdef DEBUG
    for(int r = 1; r <= 4; ++r) {
        for(int c = 1; c <= 4; ++c) {
            bool smelly = false;
            for(int ns = s; ns < MAX_REP && ns <= s + 2; ++ns) {
                if(fish_smell[r][c][ns]) {
                    smelly = true;
                    cout << ns;
                }
            }
...
void print_fish(vector<int> fish[MAX_GRID + 1][MAX_GRID + 1]) {
#ifdef DEBUG
    for(int r = 1; r <= 4; ++r) {
        for(int c = 1; c <= 4; ++c) {
            if(fish[r][c].empty()) {
...
void print_grid(int grid[MAX_GRID + 1][MAX_GRID + 1]) {
#ifdef DEBUG
    for(int r = 1; r <= 4; ++r) {
        for(int c = 1; c <= 4; ++c) {
            cout << grid[r][c] << '\t';
...
// #3
//bool visited[MAX_GRID + 1][MAX_GRID + 1];
int dir_trace[3];
int max_fish_ctr;
int min_dir_trace[3];
// 상은 1, 좌는 2, 하는 3, 우는 4로 변환한다.
vector<int> prioritized_dir = {UP, LEFT, DOWN, RIGHT};
int dir_to_int[] = {0, 2, 0, 1, 0, 4, 0, 3, 0};
int convert_dir_to_int(int dir[3]) {
    int int_ = 0;
    for(int i = 0, m = 100; i < 3; ++i, m *= 10) {
        int_ += dir_to_int[dir[i]] * m;
    }
    return int_;
}
vector<int> fish_to_eat[MAX_GRID + 1][MAX_GRID + 1];
void shark_travel(int r, int c, int move_ctr, int fish_ctr) {
    ...
    if(move_ctr >= 3) {
        ...
        goto reset_visited;   // NOTE: return해버리면 visited reset이 안 됨...
    } 
    ...
    for(int s = 0; s < S; ++s) {
        ...
        for(int r = 1; r <= 4; ++r) {
            for(int c = 1; c <= 4; ++c) {
                ...
                max_fish_ctr = -1;
                copy_grid(fish_to_eat, fish);
                shark_travel(r, c, 0, 0);

            #ifdef DEBUG
                cout << max_fish_ctr << endl;
            #endif
